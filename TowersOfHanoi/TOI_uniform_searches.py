...
# Function to move topmost disc from one peg to other
def move(st1,st2):
...
# Function to move the discs from one peg to other and add the new resultant new node to states traversed
def moveDisc(n):
    global noOfPegs
    stacks=[]
    ...
    for x in range(0,noOfPegs):
        for y in range(0,noOfPegs):
            ...
            #print('Initail stacks is', stacks)
...
# Printing the path - tracing from the resultant back to parent and print the resultant path
def printPath(node):
...
# Reading the initial and final states from the input
